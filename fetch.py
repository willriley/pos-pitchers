...
class GameData:
    ...
    """Whether we should log this game."""
      ...
      is_blowout = False
       for i in range(START_INNING, START_INNING + len(self.home_score_after)):
            is_blowout |= (
                abs(home_score_after[i] - away_score_after[i]) >= RUN_THRESHOLD)
    ...
    return [self.date, self.home_team, self.away_team, str(self.is_pos)[0],
            ...
            f"{self.home_score_after[6]-self.away_score_after[6]}",
            f"{self.home_score_after[7]-self.away_score_after[7]}",
            f"{self.home_score_after[8]-self.away_score_after[8]}",
            f"{self.home_score_after[9]-self.away_score_after[9]}",
            f"{self.home_score_after[self.innings]-self.away_score_after[self.innings]}",
            f"{self.home_pitchers_after[6]-self.away_pitchers_after[6]}",
            f"{self.home_pitchers_after[7]-self.away_pitchers_after[7]}",
            f"{self.home_pitchers_after[8]-self.away_pitchers_after[8]}",
            f"{self.home_pitchers_after[9]-self.away_pitchers_after[9]}",
            f"{self.home_pitchers_after[self.innings]-self.away_pitchers_after[self.innings]}"]
...
for game in games:
    if game['status'] != 'Final':
        gid = game['game_id']
        game_data = GameData(
            game['game_date'], game['home_name'], game['away_name'])

        linescore = statsapi.linescore(gid)
        game_data.inning = game['current_inning']
        game_data.home_score_after, game_data.away_score_after = parse_line(
            linescore, game_data.inning)
